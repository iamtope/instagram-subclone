...
const { SOCIAL_SENDGRID_API_KEY, SOCIAL_HOST_URL, SOCIAL_SENDGRID_EMAIL } = config;
...
sgMail.setApiKey(SOCIAL_SENDGRID_API_KEY);
...
class Email {
  ...
  static async sendVerificationMail(options) {
    const link = `${SOCIAL_HOST_URL}/login?token=${options.token}`;
    ...
    const message = {
      to: options.email,
      from: SOCIAL_SENDGRID_EMAIL, // Use the email address or domain you verified above
      subject: 'Welcome to Social App, Verify Your Email to Get Started',
      html: emailTemplate
    };
  ...
  static async sendResetPasswordEmail(options) {
    const link = `${SOCIAL_HOST_URL}/reset-password?token=${options.token}`;
    const mailGen = configMailGen();
    const emailFormat = await resetPasswordMailGenFormat(link);
    const emailTemplate = mailGen.generate(emailFormat);
    const message = {
      to: options.email,
      from: SOCIAL_SENDGRID_EMAIL,
